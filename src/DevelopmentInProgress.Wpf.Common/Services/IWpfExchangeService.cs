...
namespace DevelopmentInProgress.Wpf.Common.Services
{
    public interface IWpfExchangeService
    {
        ...
        Task<string> CancelOrderAsync(Interface.User user, string symbol, string orderId, string newClientOrderId = null, long recWindow = 0, CancellationToken cancellationToken = default(CancellationToken));
        Task<Account> GetAccountInfoAsync(string apiKey, string apiSecret, CancellationToken cancellationToken);
        Task<IEnumerable<Interface.AccountTrade>> GetAccountTradesAsync(Interface.User user, string symbol, DateTime startDate, DateTime endDate, long recWindow = 0, CancellationToken cancellationToken = default(CancellationToken));
        ...
        Task<IEnumerable<Interface.Trade>> GetTradesAsync(string symbol, int limit, CancellationToken cancellationToken);
        ...
        Task<IEnumerable<Candlestick>> GetCandlesticksAsync(string symbol, Interface.CandlestickInterval interval, DateTime startTime, DateTime endTime, int limit = default(int), CancellationToken token = default(CancellationToken));
        void SubscribeStatistics(IEnumerable<Symbol> symbols, Action<Exception> exception, CancellationToken cancellationToken);
        void SubscribeCandlesticks(string symbol, Interface.CandlestickInterval candlestickInterval, int limit, Action<CandlestickEventArgs> callback, Action<Exception> exception, CancellationToken cancellationToken);
